...
def eval_trajectories(dataset):
    if dataset == 'TUM_RGBD':
        return {
            ...
            # 'TUM_RGBD': ['rgbd_dataset_freiburg1_desk']
        }[dataset]
...
def dynamic_trajectory(dataset):
    if dataset == 'TUM_RGBD':
        return {
            'TUM_RGBD': ['rgbd_dataset_freiburg3_walking_xyz',
...
def create_eval_loaders(options, eval_type, keyframes, 
    ...
    if trajectory == '': 
        ...
        trajectories = dynamic_trajectory(options.dataset) 
    else: 
...
def test_TrustRegion(options):
    ...
    output_dir = osp.join(
        options.eval_set+'_results', checkpoint_name, 'summary'
    )
    final_pd.to_csv('{:}_{:}.csv'.format(output_dir, options.trajectory))
