...
from models.algorithms import DirectSolverNet, DeepRobustEstimator
...
class LeastSquareTracking(nn.Module):
    ...
    def __init__(self,
        ...
        #                                    solver_func=solver_funcs[3],
        #                                    timers=timers)
        ## TrustRegionInverseWUncertainty
    ...
    def forward(self, img0, img1, depth0, depth1, K, pose=None, init_only=False,
        ...
        preprocessed_data = self._preprocess(img0, img1, depth0, depth1,
                                             poseI=pose, obj_mask0=obj_mask0, obj_mask1=obj_mask1)
        ...
        with torch.no_grad():
            if logger is not None or vis or self.vis_feat_uncer:
                feat_0 = visualize.visualize_feature_channels(x0[0], rgb=I0, order='CHW')
                feat_1 = visualize.visualize_feature_channels(x1[0], rgb=I1, order='CHW')
                if self.uncertainty_type != 'None':
                    uncertainty_0 = visualize.visualize_feature_channels(sigma0[0], rgb=I0, order='CHW', cmap='NORMAL')
                    uncertainty_1 = visualize.visualize_feature_channels(sigma1[0], rgb=I1, order='CHW', cmap='NORMAL')
                if self.mEst_type != 'None':
                    robust_w = visualize.visualize_feature_channels(output0[1], rgb=I0, order='CHW')
                if self.combine_icp:
                    scale_w = visualize.visualize_feature_channels(output0[1], rgb=I0, order='CHW')
            if vis or self.vis_feat_uncer:
                ...
                if not self.training and index is not None:
                    ...
                    for i in levels:
                        ...
                        if f_C < 0:
                            feat_0_save = visualize.visualize_feature_channels(x0[i], order='CHW', add_ftr_avg=False)
                            feat_1_save = visualize.visualize_feature_channels(x1[i], order='CHW', add_ftr_avg=False)
                        else:
                            feat_0_save = visualize.visualize_feature_channels(
                                x0[i][0:1,f_C:f_C+1,], order='CHW',
                                add_ftr_avg=False
                            )
                            feat_1_save = visualize.visualize_feature_channels(
                                x1[i][0:1,f_C:f_C+1,], order='CHW',
                                add_ftr_avg=False
                            )
                        ...
                        if self.uncertainty_type != 'None':
                            if f_C < 0:
                                uncertainty_0_save = visualize.visualize_feature_channels(
                                    sigma0[i], order='CHW',
                                    add_ftr_avg=False
                                )
                                uncertainty_1_save = visualize.visualize_feature_channels(
                                    sigma1[i], order='CHW',
                                    add_ftr_avg=False
                                )
                            else:
                                uncertainty_0_save = visualize.visualize_feature_channels(
                                    sigma0[i][0:1,f_C:f_C+1,],
                                    order='CHW', add_ftr_avg=False
                                )
                                uncertainty_1_save = visualize.visualize_feature_channels(
                                    sigma1[i][0:1,f_C:f_C+1,],
                                    order='CHW', add_ftr_avg=False
                                )
                    ...
                    if self.mEst_type != 'None':
                        robust_w = visualize.visualize_feature_channels(output0[1], add_ftr_avg=False, order='CHW')
            ...
            if logger is not None:
                if self.training:
                    logger.add_images_to_tensorboard([feat_0, feat_1],
                                                     ...
                                                     iteration)
                    if self.uncertainty_type != 'None':
                        logger.add_images_to_tensorboard(
                            [uncertainty_0, uncertainty_1],
