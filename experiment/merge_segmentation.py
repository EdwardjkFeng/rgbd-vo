...
from geometry.geometry import compute_vertex, compute_normal, generate_xy_grid, coord_grid
...
def calculate_iou(segment1, segment2):
    intersection = torch.logical_and(segment1, segment2)
    union = torch.logical_or(segment1, segment2)
    denom = torch.min(torch.sum(segment1), torch.sum(union))
    # iou = torch.sum(intersection) / denom
    iou = torch.sum(intersection) / torch.sum(segment1)
    return iou

def merge_segmentations(masks1, masks2, iou_threshold=0.8):
    unique_classes_masks1 = torch.unique(masks1)
    unique_classes_masks2 = masks2.shape[0]
    # merge_masks = torch.zeros_like(masks1)
    merge_masks = torch.clone(masks2).float()

    # morphology
    kernel = torch.ones(3, 3).cuda()
    merge_masks = erosion(merge_masks, kernel, engine='convolution')
    
    for id1 in unique_classes_masks1[unique_classes_masks1 != 0]:
        segment1 = (masks1 == id1)
        max_iou = 0
        max_id = None
        for id2 in range(unique_classes_masks2):
            segment2 = (masks2[id2] != 0)
            iou = calculate_iou(segment1, segment2)
            if iou >= iou_threshold:
                if iou > max_iou:
                    max_iou = iou
                    max_id = id2
        if max_id is not None:
            merge_masks[max_id:max_id+1][segment1] = 1

    # morphology
    # kernel = torch.ones(9, 9).cuda()
    # merge_masks = dilation(merge_masks, kernel, engine='convolution')
...
def create_random_labels_map(classes) -> dict[int, tuple[int, int, int]]:
...
if __name__ == '__main__':
    ...
    maskrcnn = MaskRCNN(target_labels=[1])
    ...
    with torch.no_grad():
        for batch in torch_loader:
            ...
            edge_segm_with_edge = connected_components(
                edginess_map1, num_iterations=1000
            )
            edge_segm = edge_segm_with_edge[None].float()

            # Dilation to remove edges
            kernel = torch.ones((3, 3)).cuda()
            for i in range(3):
                edge_segm = dilation(edge_segm, kernel)

            labels_im1 = edge_segm_with_edge.squeeze().cpu()
            ...
            labels_im1 = labels_to_image(labels_im1, labels_map1)
            labels_im1 = labels_im1.permute(1, 2, 0).squeeze().numpy()
            ...
            # # Segment the image
            # edge_segm = connected_components(edginess_map2, num_iterations=1000)
            # labels_im2 = edge_segm.squeeze().cpu()
            # # Covert labels to labeled image
            # color_ids2, pix_counts = torch.unique(labels_im2, return_counts=True)
            # # sparse_classes = color_ids2[pix_counts < 100]
            # # for cls in sparse_classes:
            # #     labels_im2[labels_im2 == cls] = 0
            ...
            rcnn_segm, rcnn_boxes = maskrcnn(color0)

            merge_segm = merge_segmentations(edge_segm, rcnn_segm[0], iou_threshold=0.35)
            
            masked_img = draw_segmentation_masks(
                (color0 * 255).squeeze().to(torch.uint8).cpu(),
                merge_segm.to(bool).squeeze().cpu(),
                alpha=0.9,
            )

            masked_img = transform_F.to_pil_image(masked_img)
            masked_img = np.asarray(masked_img)

            cv2.namedWindow("Merge", cv2.WINDOW_NORMAL)
            cv2.imshow("Merge", create_mosaic([masked_img], cmap="NORMAL", normalize=True))
